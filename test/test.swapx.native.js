...
var swapx = tryRequire( resolve( __dirname, './../lib/swapx.native.js' ) );
var opts = {
	'skip': ( swapx instanceof Error )
};
...
tape( 'the function has an arity of 6', opts, function test( t ) {
	t.strictEqual( swapx.length, 6, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	w = new Float32Array( x.length );
	out = swapx( x.length, 3.0, x, 1, w, 1 );
...
tape( 'the function supports view offsets', opts, function test( t ) {
	...
	w1 = new Float32Array( w0.buffer, w0.BYTES_PER_ELEMENT*2 );
